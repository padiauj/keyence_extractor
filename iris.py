...
def extract_positions(positions_path):
    ...
    with open(positions_path, 'r') as f:
        for line in f:
            ...
            positions.append({
                "number":
                file_to_number(from_colon_format(fields[0])),
                "corr":
                from_colon_format(fields[1]),
                "position":
                make_tuple(from_colon_format(fields[2])),
                "grid":
                from_colon_format(fields[3])
            })
    return positions
...
def prep_image_folders(src_dir, stitch_dir, pattern_format, channels):
    ...
    for channel in channels:
        ...
        if (channel == "Overlay"):
            ...
            logger.info(
                "Generating 16-bit grayscale images for channel %s ..." %
                (channel))
...
def mist(image_dir, output_dir, rows, cols, pattern):
    ...
    pos = extract_positions(
        os.path.join(output_dir, "img-global-positions-1.txt"))
    encode_positions(pos, os.path.join(output_dir, "stitching.csv"))
    for f in glob.glob(os.path.join(output_dir, "img-*")):
        os.remove(f)
    return pos
...
def blend_images(positions, stitch_dir, prefix, channel):
    ...
    for idx, pos in enumerate(positions):
        ...
        corner = pos['position']
        section = output[corner[1]:corner[1] + size[1], corner[0]:corner[0] +
                         size[0]]
        section[section == 0] = im[section == 0]
        section[section != 0] = (section[section != 0] + im[section != 0]) / 2
    cv2.imwrite(os.path.join(stitch_dir, channel, channel + "_stitched.png"),
                output)
...
if __name__ == "__main__":
    ...
    logger.info("Discovered image sequence with %s rows and %s columns" %
                (rows, cols))
